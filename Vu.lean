...
theorem natLtB_irrefl (n : Nat) : natLtB n n = Bool.false :=
  Nat.recOn (motive := fun m => natLtB m m = Bool.false) n
    (Eq.refl Bool.false)
    (fun _ ih => ih)
theorem natLeB_succ_self (n : Nat) : natLeB n (Nat.succ n) = Bool.true :=
  Nat.recOn (motive := fun m => natLeB m (Nat.succ m) = Bool.true) n
    (Eq.refl Bool.true)
    (fun _ ih => ih)
theorem natLtB_succ_self (n : Nat) : natLtB n (Nat.succ n) = Bool.true :=
  natLeB_refl (Nat.succ n)

theorem natSub_self (n : Nat) : natSub n n = 0 :=
  Nat.recOn (motive := fun m => natSub m m = 0) n
    (Eq.refl 0)
    (fun m ih =>
      let step : natSub (Nat.succ m) (Nat.succ m) = natPred (natSub (Nat.succ m) m) := Eq.refl _
      Nat.recOn
        (motive := fun k => natSub k k = 0 → natSub (Nat.succ k) k = 1 ∨ natSub (Nat.succ k) k = Nat.succ 0 → natSub (Nat.succ m) (Nat.succ m) = 0)
        m ih (fun _ _ => ih) ih (Or.inl (Eq.refl 1)))

theorem natSub_succ_self (n : Nat) : natSub (Nat.succ n) n = 1 :=
  Nat.recOn (motive := fun m => natSub (Nat.succ m) m = 1) n
    (Eq.refl 1)
    (fun m ih =>
      let step : natSub (Nat.succ (Nat.succ m)) (Nat.succ m)
               = natPred (natSub (Nat.succ (Nat.succ m)) m) := Eq.refl _
      Eq.trans step (congrArg natPred
        (Nat.recOn (motive := fun k =>
            natSub (Nat.succ (Nat.succ m)) k = Nat.succ (natSub (Nat.succ m) k)) m
          (Eq.refl (Nat.succ (Nat.succ m)))
          (fun k ihk =>
            let s1 : natSub (Nat.succ (Nat.succ m)) (Nat.succ k)
                   = natPred (natSub (Nat.succ (Nat.succ m)) k) := Eq.refl _
            let s2 : natSub (Nat.succ m) (Nat.succ k)
                   = natPred (natSub (Nat.succ m) k) := Eq.refl _
            Eq.trans s1
              (Eq.trans (congrArg natPred ihk)
                (Eq.symm (congrArg Nat.succ s2 ▸ Eq.refl _)))))))
...
def FixedQ.leB (a b : FixedQ) : Bool :=
  bOr (FixedQ.ltB a b) (FixedQ.eqB a b)
...
theorem FixedQ.neg_zero : FixedQ.neg FixedQ.zero = FixedQ.zero := Eq.refl _
...
theorem FixedQ.leB_refl (x : FixedQ) : FixedQ.leB x x = Bool.true :=
  let h := FixedQ.eqB_refl x
  Eq.trans (congrArg (fun b => bOr (FixedQ.ltB x x) b) h)
    (bOr_true_r (FixedQ.ltB x x))
...
theorem Tensor.initFromData2D_data (rows cols : Nat) (d : List FixedQ) :
    Tensor.data (Tensor.initFromData2D rows cols d) = d := Eq.refl d
...
theorem Tensor.sameShape_self (t : Tensor)
    (hdim : natEqB (Tensor.dimsLen t) 2 = Bool.true) :
  ...
  let step_inner : bAnd (natEqB (Tensor.rows2D t) (Tensor.rows2D t))
    ...
    Eq.trans
      ...
      (Eq.trans
        (congrArg (fun x => bAnd Bool.true x) h_cols)
        (Eq.refl Bool.true))
  let step1 : Tensor.sameShape t t
    = bAnd Bool.true (bAnd Bool.true (bAnd (natEqB (Tensor.rows2D t) (Tensor.rows2D t))
                                           (natEqB (Tensor.cols2D t) (Tensor.cols2D t)))) :=
    Eq.trans
      (congrArg (fun b => bAnd b (bAnd (natEqB (Tensor.dimsLen t) 2)
        (bAnd (natEqB (Tensor.rows2D t) (Tensor.rows2D t))
              (natEqB (Tensor.cols2D t) (Tensor.cols2D t))))) hdim)
      (congrArg (fun b => bAnd Bool.true (bAnd b (bAnd (natEqB (Tensor.rows2D t) (Tensor.rows2D t))
                                                       (natEqB (Tensor.cols2D t) (Tensor.cols2D t))))) hdim)
  Eq.trans step1
    (Eq.trans (congrArg (fun x => bAnd Bool.true (bAnd Bool.true x)) step_inner)
              (Eq.refl Bool.true))
...
theorem Tensor.sameShape_refl_2D (rows cols : Nat) :
  ...
  Tensor.sameShape_self (Tensor.initZeros2D rows cols) (Eq.refl Bool.true)
...
theorem validateTensor2D_initZeros_ok (rows cols : Nat)
  ...
  let step1 : ResultT.bind (checkedMul rows cols)
      (fun expected =>
        bIte (natEqB (Tensor.dataLen t) expected)
          (ResultT.ok expected)
          (ResultT.err ZigError.dataLengthMismatch))
...
theorem validateClipRange_implies_ordered (cmin cmax : FixedQ)
  ...
  Bool.recOn
    (motive := fun bv =>
      ...
      FixedQ.ltB cmin cmax = Bool.true)
    (fun heq hk =>
      False.elim (ResultT.noConfusion hk (fun _ => boolFalseNeTrue (Eq.refl _))))
    (fun heq _ => heq)
...
theorem validateComparisonTolerances_finite_always (absTol relTol : FixedQ) :
  ...
  validateClipRange_finite_always absTol relTol

theorem validateComparisonTolerances_ok_implies_nonneg (absTol relTol : FixedQ)
  ...
  let finiteStep :
      ...
      = bIte (bOr (FixedQ.ltB absTol FixedQ.zero) (FixedQ.ltB relTol FixedQ.zero))
          (ResultT.err ZigError.invalidTolerance)
          (ResultT.ok Unit.unit) :=
    congrArg (fun c => bIte c (ResultT.err ZigError.invalidTolerance)
          (bIte (bOr (FixedQ.ltB absTol FixedQ.zero) (FixedQ.ltB relTol FixedQ.zero))
            (ResultT.err ZigError.invalidTolerance)
            (ResultT.ok Unit.unit)))
  ...
  Bool.recOn
    (motive := fun bv =>
      bv = bOr (FixedQ.ltB absTol FixedQ.zero) (FixedQ.ltB relTol FixedQ.zero) →
      bIte bv (ResultT.err ZigError.invalidTolerance) (ResultT.ok Unit.unit)
      = ResultT.ok Unit.unit →
      And (FixedQ.ltB absTol FixedQ.zero = Bool.false)
          (FixedQ.ltB relTol FixedQ.zero = Bool.false))
    (fun heqOr _ =>
      let hOrFalse : bOr (FixedQ.ltB absTol FixedQ.zero) (FixedQ.ltB relTol FixedQ.zero)
                     = Bool.false := Eq.symm heqOr
      Bool.recOn
        (motive := fun ba =>
          ba = FixedQ.ltB absTol FixedQ.zero →
          bOr ba (FixedQ.ltB relTol FixedQ.zero) = Bool.false →
          And (FixedQ.ltB absTol FixedQ.zero = Bool.false)
              (FixedQ.ltB relTol FixedQ.zero = Bool.false))
        (FixedQ.ltB absTol FixedQ.zero)
        (fun heqa hor =>
          let habs : FixedQ.ltB absTol FixedQ.zero = Bool.false := Eq.symm heqa
          let hrel : FixedQ.ltB relTol FixedQ.zero = Bool.false :=
            Eq.trans (Eq.symm (bOr_false_l (FixedQ.ltB relTol FixedQ.zero))) hor
          And.intro habs hrel)
        (fun heqa hor =>
          let contra : Bool.true = Bool.false :=
            Eq.trans
              (Eq.symm (Eq.trans (congrArg (fun b => bOr b (FixedQ.ltB relTol FixedQ.zero)) (Eq.symm heqa))
                         (bOr_true_l (FixedQ.ltB relTol FixedQ.zero))))
              hor
          False.elim (boolTrueNeFalse contra))
        (Eq.refl _) hOrFalse)
    (fun _ hk => False.elim (ResultT.noConfusion hk (fun _ => boolFalseNeTrue (Eq.refl _))))
...
theorem validateModelConfigValues_ok_dim_pos (dim numLayers : Nat) (cfg : RSFConfig)
  ...
  Bool.recOn
    (motive := fun bv =>
      ...
      natEqB dim 0 = Bool.false)
    (fun heq _ => heq)
    (fun _ hv => False.elim (ResultT.noConfusion hv (fun _ => boolFalseNeTrue (Eq.refl _))))
...
theorem zipWith_preserves_length_equal (f : FixedQ → FixedQ → FixedQ)
  ...
  List.recOn
    (motive := fun x => (y : List FixedQ) → (m : Nat) →
      List.length x = m → List.length y = m →
      List.length (List.zipWith f x y) = m) a
    (fun _ m hx _ =>
      let e : m = 0 := Eq.symm hx
      Eq.trans (Eq.refl 0) e)
    (fun ha' ta ih y m =>
      List.recOn
        (motive := fun y =>
          List.length (List.cons ha' ta) = m → List.length y = m →
          List.length (List.zipWith f (List.cons ha' ta) y) = m) y
        (fun _ hy =>
          let e : m = 0 := Eq.symm hy
          Eq.trans (Eq.refl 0) e)
        (fun hb' tb _ hx hy =>
          Nat.recOn
            (motive := fun d =>
...
theorem zipWithMul_one_identity (xs : List FixedQ) (n : Nat)
  ...
  List.recOn
    ...
    (fun m hm =>
      Nat.recOn (motive := fun k =>
          0 = k → zipWithMul List.nil (List.replicate k FixedQ.one) = List.nil)
        m
    ...
    (fun hd tl ih m hm =>
      Nat.recOn (motive := fun k =>
        ...
        (fun k _ hsk =>
          ...
          let hmul_one : FixedQ.mul hd FixedQ.one = hd :=
            FixedQ.recOn (motive := fun x => FixedQ.mul x FixedQ.one = x) hd
              (fun n =>
                Nat.recOn (motive := fun p => Nat.mul p 1 = p) n
                  (Eq.refl 0)
                  (fun p ihp => congrArg Nat.succ ihp)
                |> (fun heq => congrArg FixedQ.nonneg heq))
              (fun n =>
                let heqN : Nat.mul (Nat.succ n) 1 = Nat.succ n :=
                  Nat.recOn (motive := fun p => Nat.mul p 1 = p) (Nat.succ n)
                    (Eq.refl 0)
                    (fun p ihp => congrArg Nat.succ ihp)
                let base : FixedQ.mul (FixedQ.negsucc n) FixedQ.one
                         = FixedQ.neg (FixedQ.nonneg (Nat.mul (Nat.succ n) 1)) := Eq.refl _
                let step : FixedQ.neg (FixedQ.nonneg (Nat.mul (Nat.succ n) 1))
                         = FixedQ.neg (FixedQ.nonneg (Nat.succ n)) :=
                  congrArg (fun x => FixedQ.neg (FixedQ.nonneg x)) heqN
                let final : FixedQ.neg (FixedQ.nonneg (Nat.succ n)) = FixedQ.negsucc n :=
                  Eq.refl _
                Eq.trans base (Eq.trans step final))
          let step : zipWithMul (List.cons hd tl) (List.replicate (Nat.succ k) FixedQ.one)
            ...
            Eq.refl _
          Eq.trans step
            (Eq.trans (congrArg (fun x => List.cons x (zipWithMul tl (List.replicate k FixedQ.one))) hmul_one)
...
theorem zipWithAdd_zero_identity (xs : List FixedQ) (n : Nat)
  ...
  List.recOn
    ...
    (fun m hm =>
      Nat.recOn (motive := fun k =>
          0 = k → zipWithAdd List.nil (List.replicate k FixedQ.zero) = List.nil)
        m (fun _ => Eq.refl List.nil) (fun k _ heq => False.elim (Nat.noConfusion heq)) hm)
    (fun hd tl ih m hm =>
      Nat.recOn (motive := fun k =>
          Nat.succ (List.length tl) = k →
...
theorem zipWithSub_zero_identity (xs : List FixedQ) (n : Nat)
  ...
  List.recOn
    ...
    (fun m hm =>
      Nat.recOn (motive := fun k =>
          0 = k → zipWithSub List.nil (List.replicate k FixedQ.zero) = List.nil)
        m (fun _ => Eq.refl List.nil) (fun k _ heq => False.elim (Nat.noConfusion heq)) hm)
    (fun hd tl ih m hm =>
      Nat.recOn (motive := fun k =>
          Nat.succ (List.length tl) = k →
        ...
        (fun k _ hsk =>
          ...
          let step : zipWithSub (List.cons hd tl) (List.replicate (Nat.succ k) FixedQ.zero)
            ...
            Eq.refl _
          let hsub_zero : FixedQ.sub hd FixedQ.zero = hd :=
            Eq.trans
              (congrArg (FixedQ.add hd)
                (FixedQ.recOn (motive := fun _ => FixedQ.neg FixedQ.zero = FixedQ.zero)
                  FixedQ.zero (Eq.refl FixedQ.zero) (fun _ => Eq.refl FixedQ.zero)))
              (FixedQ.add_zero hd)
          Eq.trans step
            (Eq.trans
              (congrArg (fun x => List.cons x (zipWithSub tl (List.replicate k FixedQ.zero))) hsub_zero)
              (congrArg (List.cons hd) (ih k htl))))
        hm)
    n h
...
theorem forwardRow2D_then_inverseRow2D_identity
    ...
    And
      (zipWithMul (zipWithMul x1 (List.replicate (LayerCore.dim layer) FixedQ.one))
...
theorem mergeFrom_then_splitInto_roundtrip (dim : Nat) (x1 x2 : Tensor)
    ...
    Exists (fun (tsplit : Tensor) =>
      And (splitInto dim t = ResultT.ok tsplit)
          (And (Tensor.cols2D tsplit = dim)
               (Tensor.rows2D tsplit = Tensor.rows2D x1))) :=
  ...
  let htReduce : mergeFrom dim x1 x2
                = ResultT.ok (Tensor.initFromData2D (Tensor.rows2D x1) (Nat.add dim dim)
                                (List.append (Tensor.data x1) (Tensor.data x2))) :=
    congrArg (fun c => bIte c
      (ResultT.ok (Tensor.initFromData2D (Tensor.rows2D x1) (Nat.add dim dim)
  ...
  let htEq : t = Tensor.initFromData2D (Tensor.rows2D x1) (Nat.add dim dim)
                    (List.append (Tensor.data x1) (Tensor.data x2)) :=
    ResultT.ok.inj (Eq.trans (Eq.symm hmerge) htReduce)
  let tmerged := Tensor.initFromData2D (Tensor.rows2D x1) (Nat.add dim dim)
                    (List.append (Tensor.data x1) (Tensor.data x2))
  let hcolsMerged : Tensor.cols2D tmerged = Nat.add dim dim := Eq.refl _
  let hdimsMerged : Tensor.dimsLen tmerged = 2 := Eq.refl _
  let heqCond : bAnd (natEqB (Tensor.cols2D tmerged) (Nat.add dim dim))
                     (natEqB (Tensor.dimsLen tmerged) 2) = Bool.true :=
    Eq.trans
      (congrArg (fun c => bAnd (natEqB c (Nat.add dim dim)) (natEqB (Tensor.dimsLen tmerged) 2))
        hcolsMerged)
      (Eq.trans
        (congrArg (fun c => bAnd (natEqB (Nat.add dim dim) (Nat.add dim dim)) c)
          (Eq.refl (natEqB (Tensor.dimsLen tmerged) 2)))
        (let h1 := natEqB_refl (Nat.add dim dim)
         Eq.trans (congrArg (fun b => bAnd b (natEqB (Tensor.dimsLen tmerged) 2)) h1)
           (Eq.refl Bool.true)))
  let tsplit := Tensor.initFromData2D (Tensor.rows2D tmerged) dim (Tensor.data tmerged)
  let hSplitReduce : splitInto dim tmerged = ResultT.ok tsplit :=
    congrArg (fun c => bIte c (ResultT.ok tsplit) (ResultT.err ZigError.shapeMismatch)) heqCond
  let hSplitT : splitInto dim t = ResultT.ok tsplit :=
    Eq.trans (congrArg (splitInto dim) htEq) hSplitReduce
  Exists.intro tsplit
    (And.intro hSplitT
      (And.intro (Eq.refl dim) (Eq.refl (Tensor.rows2D x1))))
...
def inverseOnCoreFull (layers : List LayerCore) (y : Tensor) : ResultT Tensor :=
  List.recOn (motive := fun _ => ResultT Tensor) (List.reverse layers)
    (ResultT.ok y)
    (fun layer _ ih =>
      ResultT.bind ih (fun acc =>
        ResultT.bind (splitInto (LayerCore.dim layer) acc) (fun half =>
  ...
  List.recOn (motive := fun _ => List FixedQ → Bool) xs
    ...
    (fun x xt ihx ys2 =>
      List.recOn (motive := fun _ => Bool) ys2
        Bool.false
        (fun y yt _ =>
          bAnd (bOr (FixedQ.eqB x y) (FixedQ.leB (FixedQ.absDiff x y) absTol))
               (ihx yt)))
    ys
...
theorem listAllClose_refl (xs : List FixedQ) (tol : FixedQ) :
  ...
  List.recOn
    ...
    (Eq.refl Bool.true)
    (fun x xt ih =>
      let heq : FixedQ.eqB x x = Bool.true := FixedQ.eqB_refl x
      let hor : bOr (FixedQ.eqB x x) (FixedQ.leB (FixedQ.absDiff x x) tol) = Bool.true :=
        Eq.trans (congrArg (fun b => bOr b (FixedQ.leB (FixedQ.absDiff x x) tol)) heq)
          (bOr_true_l _)
      let step : listAllClose (List.cons x xt) (List.cons x xt) tol
               = bAnd (bOr (FixedQ.eqB x x) (FixedQ.leB (FixedQ.absDiff x x) tol))
                      (listAllClose xt xt tol) := Eq.refl _
      Eq.trans step
        (Eq.trans (congrArg (fun b => bAnd b (listAllClose xt xt tol)) hor)
          (Eq.trans (congrArg (fun b => bAnd Bool.true b) ih) (Eq.refl Bool.true))))

theorem tensorAllCloseEq_refl (t : Tensor) (tol : FixedQ)
    (hdim : natEqB (Tensor.dimsLen t) 2 = Bool.true) :
    tensorAllCloseEq t t tol = Bool.true :=
  let hshape : Tensor.sameShape t t = Bool.true :=
    Tensor.sameShape_self t hdim
  let hclose : listAllClose (Tensor.data t) (Tensor.data t) tol = Bool.true :=
    listAllClose_refl (Tensor.data t) tol
  Eq.trans (congrArg (fun x => bAnd x (listAllClose (Tensor.data t) (Tensor.data t) tol)) hshape)
...
def syncAllLayersGPU (core : Nat) (_buf : GPUBuffer) : GPUBuffer :=
...
theorem snapshotModelForSave_preserves_layer_count
  ...
  ResultT.recOn
    ...
    (fun _ _ hk => False.elim (ResultT.noConfusion hk (fun _ => boolFalseNeTrue (Eq.refl _))))
    (Eq.refl _) h
...
def writeSnapshotVersion4Bytes (_s : SavedModelSnapshot) : List Nat :=
...
def backwardOnCore (_layers : List LayerCore) (gradOutput input : Tensor) :
    ResultT Tensor :=
...
theorem registry_acquire_after_destroy (r : LayerRegistry) (id : Nat)
    ...
    (hk : LayerRegistry.lookup r id = ResultT.ok baseEntry)
...
def chainRule (_layer : LayerCore) (dy : List FixedQ) : List FixedQ :=
...
def composeForwardBackward
    (_layer : LayerCore) (_x1 _x2 dy1 dy2 : List FixedQ) : List FixedQ :=
  zipWithAdd dy1 dy2
...
theorem layerCore_initOwned_dim (dim : Nat) (cfg : RSFLayerConfig) (l : LayerCore)
    ...
    LayerCore.dim l = dim :=
  let hdim : natEqB dim 0 = Bool.false :=
    LayerCore.initOwned_ok_implies_dim_pos dim cfg l h
  let afterDim :
            ...
            (fun _ =>
              ResultT.ok
                (LayerCore.mk
                  ...
                  (RSFLayerConfig.clipMin cfg)
                  (RSFLayerConfig.clipMax cfg)
                  (RSFLayerConfig.gradMean cfg)))))) hdim
  let h2 := Eq.trans (Eq.symm afterDim) h
  ResultT.recOn
    (motive := fun rv =>
            ...
            (fun _ =>
              ResultT.ok
                (LayerCore.mk
                  ...
                  (RSFLayerConfig.clipMax cfg)
                  (RSFLayerConfig.gradMean cfg))))
      = ResultT.ok l → LayerCore.dim l = dim)
        ...
        (motive := fun rv =>
          ...
          ResultT.bind rv (fun _ => ResultT.ok
            (LayerCore.mk
              ...
              (RSFLayerConfig.clipMin cfg)
              (RSFLayerConfig.clipMax cfg)
              (RSFLayerConfig.gradMean cfg))) = ResultT.ok l → LayerCore.dim l = dim)
        (checkedMul dim dim)
        (fun _ _ hkk =>
          let heq : LayerCore.mk
              (Tensor.initZeros2D dim dim)
              (Tensor.initZeros2D dim dim)
              (Tensor.initZeros2D 1 dim)
              (Tensor.initZeros2D 1 dim)
              ...
              (RSFLayerConfig.clipMin cfg)
              (RSFLayerConfig.clipMax cfg)
              (RSFLayerConfig.gradMean cfg) = l := ResultT.ok.inj hkk
          Eq.trans (Eq.symm (congrArg LayerCore.dim heq)) (Eq.refl dim))
        (fun _ _ hkk => False.elim (ResultT.noConfusion hkk (fun _ =>
          boolFalseNeTrue (Eq.refl _))))
        (Eq.refl _) hk)
...
def copyHandle (_s : HandleState) : HandleState :=
  HandleState.copied

theorem acquireHandle_ok_implies_dim_pos (dim : Nat) (cfg : RSFLayerConfig) (s : HandleState)
    (h : acquireHandle dim cfg = ResultT.ok s) :
    natEqB dim 0 = Bool.false :=
  ResultT.recOn
    (motive := fun rv =>
      rv = LayerCore.initOwned dim cfg →
      ResultT.bind rv (fun layer => ResultT.ok (HandleState.initialized layer GradState.noGrad))
      = ResultT.ok s →
      natEqB dim 0 = Bool.false)
    (LayerCore.initOwned dim cfg)
    (fun l heq _ =>
      LayerCore.initOwned_ok_implies_dim_pos dim cfg l (Eq.symm heq))
    (fun _ _ hk => False.elim (ResultT.noConfusion hk (fun _ => boolFalseNeTrue (Eq.refl _))))
    (Eq.refl _) h
...
inductive DeserializedModel : Type where
  | mk : Nat → Nat → List (List FixedQ) → DeserializedModel

def DeserializedModel.dim (d : DeserializedModel) : Nat :=
  DeserializedModel.recOn (motive := fun _ => Nat) d (fun x _ _ => x)
def DeserializedModel.numLayers (d : DeserializedModel) : Nat :=
  DeserializedModel.recOn (motive := fun _ => Nat) d (fun _ x _ => x)
def DeserializedModel.layerData (d : DeserializedModel) : List (List FixedQ) :=
  DeserializedModel.recOn (motive := fun _ => List (List FixedQ)) d (fun _ _ x => x)

def deserializeModel (b : ModelBlob) : ResultT DeserializedModel :=
  bIte (natEqB (ModelBlob.version b) 1)
    (bIte (natEqB (ModelBlob.dim b) 0)
      (ResultT.err ZigError.invalidDimension)
      (bIte (natEqB (ModelBlob.numLayers b) 0)
        (ResultT.err ZigError.invalidLayerCount)
        (ResultT.ok (DeserializedModel.mk
          (ModelBlob.dim b) (ModelBlob.numLayers b) (ModelBlob.layerData b)))))
...
theorem saveLoad_version_roundtrip (dim numLayers : Nat) (layers : List LayerCore)
    ...
    Exists (fun (result : DeserializedModel) =>
      And (deserializeModel (serializeModel dim numLayers layers) = ResultT.ok result)
          (And (DeserializedModel.dim result = dim)
               (DeserializedModel.numLayers result = numLayers))) :=
  let mapped := List.map (fun l => List.append (Tensor.data (LayerCore.sWeight l))
                         (List.append (Tensor.data (LayerCore.tWeight l))
                           (List.append (Tensor.data (LayerCore.sBias l))
                             (Tensor.data (LayerCore.tBias l))))) layers
  let res := DeserializedModel.mk dim numLayers mapped
  ...
  let step2 : bIte (natEqB dim 0)
    ...
    congrArg (fun c => bIte c
      ...
      (bIte (natEqB numLayers 0)
        (ResultT.err ZigError.invalidLayerCount)
        (ResultT.ok res))) hdim
  let step3 : bIte (natEqB numLayers 0)
                (ResultT.err ZigError.invalidLayerCount)
                (ResultT.ok res)
            = ResultT.ok res :=
    congrArg (fun c => bIte c
      (ResultT.err ZigError.invalidLayerCount)
      (ResultT.ok res)) hlayers
  Exists.intro res
    (And.intro
      (Eq.trans step1 (Eq.trans step2 step3))
...
theorem modelChecksum_nil :
    modelChecksum List.nil = 0 := Eq.refl 0

theorem simpleChecksum_nil : simpleChecksum List.nil = 0 := Eq.refl 0

theorem listNatOfFixedQ_length (xs : List FixedQ) :
    List.length (listNatOfFixedQ xs) = List.length xs :=
  List.recOn
    (motive := fun l =>
      List.length (List.map (fun x =>
        FixedQ.recOn (motive := fun _ => Nat) x (fun n => n) (fun n => Nat.succ n)) l)
      = List.length l)
    xs
    (Eq.refl 0)
    (fun _ _ ih => congrArg Nat.succ ih)
